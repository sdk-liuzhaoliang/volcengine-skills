...
If explicitly approved, record every returned resource ID, use `DeleteCluster` with an explicit retention/deletion policy, and verify `ListClusters` no longer returns the test cluster.
